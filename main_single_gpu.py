...
def main():
    ...
    avg_acc1 = 0.
    best_val_acc1 = 0.
    the_best_model_epoch = 0
    the_validation_requirement = config.VALIDATION.REQUIREMENTS
    
    # state_param_dict = paddle.load('Best_CycleMLP.pdparams')
    # state_opt_dict = paddle.load('Best_CycleMLP.pdopt')
    # optimizer.set_state_dict(state_opt_dict)
    # model.set_state_dict(state_param_dict)
    ...
    for epoch in range(last_epoch+1, config.TRAIN.NUM_EPOCHS+1):
        ...
        if best_val_acc1 > 0.:
            logger.info(f"----- Epoch[{epoch:03d}/{config.TRAIN.NUM_EPOCHS:03d}], " +
                        ...
                        f"Best Val(Till Now) Acc@1: {best_val_acc1:.4f}")
        else:
            logger.info(f"----- Epoch[{epoch:03d}/{config.TRAIN.NUM_EPOCHS:03d}], " +
                            f"Train Loss: {train_loss:.4f}, " +
                            f"Train Acc: {train_acc:.4f}, " +
        ...
        if epoch % config.VALIDATE_FREQ == 0 or epoch == config.TRAIN.NUM_EPOCHS:
            ...
            logger.info(f"----- Epoch[{epoch:03d}/{config.TRAIN.NUM_EPOCHS:03d}], " +
                        ...
                        f"time: {val_time:.2f}")
        # model save
        if avg_acc1 > best_val_acc1:
            model_path = os.path.join(
                config.SAVE, f"Best_{config.MODEL.TYPE}")
            paddle.save(model.state_dict(), model_path + '.pdparams')
            paddle.save(optimizer.state_dict(), model_path + '.pdopt')
            logger.info(f"the pre best model acc:{best_val_acc1:.4f}, at epoch {the_best_model_epoch}")
            logger.info(f"current best model acc:{avg_acc1:.4f}, at epoch {epoch}")
            logger.info(f"----- Save BEST model: {model_path}.pdparams")
            logger.info(f"----- Save BEST optim: {model_path}.pdopt")
            best_val_acc1 = avg_acc1
            the_best_model_epoch = epoch
                
            # ends when the best model meets the validation accuracy requirements
            if best_val_acc1 > the_validation_requirement + 0.01:
                logger.info(f"the accuracy is satisfied. training ends")
                break
