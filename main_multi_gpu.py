...
def main_worker(*args):
    ...
    for epoch in range(last_epoch+1, config.TRAIN.NUM_EPOCHS+1):
        ...
        # print the current best val acc1 info
        if best_val_acc1 > 0.:
        ...
        if local_rank == 0:
            if avg_acc1 > best_val_acc1:
                master_logger.info(f"----- Epoch[{epoch:03d}/{config.TRAIN.NUM_EPOCHS:03d}], " +
                            ...
                            f"time: {train_time:.2f}, " +
                            f"Best Val(epoch{epoch}) Acc@1: {avg_acc1:.4f}")
            elif best_val_acc1 > 0.:
                master_logger.info(f"----- Epoch[{epoch:03d}/{config.TRAIN.NUM_EPOCHS:03d}], " +
                            f"Train Loss: {avg_loss:.4f}, " +
                            f"Train Acc: {avg_acc:.4f}, " +
                            f"time: {train_time:.2f}, " +
                            f"Best Val(epoch{the_best_model_epoch}) Acc@1: {best_val_acc1:.4f}")
            else:
                master_logger.info(f"----- Epoch[{epoch:03d}/{config.TRAIN.NUM_EPOCHS:03d}], " +
                                f"Train Loss: {avg_loss:.4f}, " +
                                f"Train Acc: {avg_acc:.4f}, " +
        ...
        if local_rank == 0:
            if avg_acc1 > best_val_acc1:
                ...
                master_logger.info(f"the pre best model acc:{best_val_acc1:.4f}, at epoch {the_best_model_epoch}")
                master_logger.info(f"current best model acc:{avg_acc1:.4f}, at epoch {epoch}")
                ...
                if best_val_acc1 > the_validation_requirement + 0.01:
                    master_logger.info(f"the accuracy is satisfied. training ends")
                    break
